...
- The client default is `transport_mode=apps_script` for fronted operation.
  `transport_mode=auto` is opt-in and prefers direct stream when configured.
  For POST, `auto` chooses one transport: direct POST via `relay_urls` when
  present, otherwise Apps Script via `script_keys`. Direct POST and Apps Script
  are not currently mixed as two POST fallback tiers.
...
- Google documents Apps Script quotas as per-user/account limits, so deployments
  under the same Google account should be treated as sharing quota. Use account
