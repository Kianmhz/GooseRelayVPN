...
func (c *Client) runStreamLoop(ctx context.Context) {
	...
	for ctx.Err() == nil {
		...
		if err := c.runStreamConnection(ctx, url); err != nil && ctx.Err() == nil {
			...
			log.Printf("[carrier] direct stream %s unavailable: %s", shortScriptKey(url), safeLogError(err))
			if !c.postEnabled() {
...
func (c *Client) runStreamConnection(ctx context.Context, url string) error {
	...
	log.Printf("[carrier] direct stream connected via %s", shortScriptKey(url))

	helloFrames, err := c.streamHelloFrames()
	if err != nil {
		return fmt.Errorf("build stream hello: %w", err)
	}
	hello, err := c.encodeStreamBatch(helloFrames)
	if err != nil {
...
func (c *Client) readStream(ctx context.Context, conn *websocket.Conn) error {
	for {
		...
		start := time.Now()
		clientID, rxFrames, err := frame.DecodeBatchBinary(c.aead, body)
...
func (c *Client) writeStream(ctx context.Context, conn streamWriter) error {
	ping := time.NewTicker(c.streamPingInterval)
	defer ping.Stop()
	for {
		frames, drainedIDs, drainSnaps := c.drainAllForStream()
		if len(frames) > 0 {
...
func (c *Client) streamHelloFrames() ([]*frame.Frame, error) {
	if !c.freshStartReset || c.clientInstanceID == "" {
		return nil, nil
	}
	...
	return []*frame.Frame{probeFrame}, nil
}
